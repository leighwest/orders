...
public class OrderReceivedEmailSender {
    ...
    public void send(Order order) {
        ...
        try {
            ...
            log.error("Error sending email for order ID: {}. Error message: {}", order.getId(), ex.getMessage());
        } catch (MessagingException ex) {
            log.error("Error sending email for order ID: {}. MessagingException: {}", order.getId(), ex.getMessage());
    ...
    private Mail getMail(Order order) {
        ...
        return new Mail("leighwest@hotmail.com", "leigh.west.aep@gmail.com", "Your cupcake order",
                emailBody, true);
    }
}
